...
for task in subtasks:
    ...
    cov = np.zeros((3*nbins, 3*nbins))

    cov[:nbins, :nbins] = so_cov.bin_mat(coupling_dict["TaTcTbTd"] * so_cov.chi(na, nc, nb, nd, ns, l, ps_all, nl_all, "TTTT"), binning_file, lmax)
    cov[:nbins, :nbins] += so_cov.bin_mat(coupling_dict["TaTdTbTc"] * so_cov.chi(na, nd, nb, nc, ns, l, ps_all, nl_all, "TTTT"), binning_file, lmax)
           
    cov[nbins:2*nbins, nbins:2*nbins] = so_cov.bin_mat(coupling_dict["TaTcPbPd"] * so_cov.chi(na, nc, nb, nd, ns, l, ps_all, nl_all, "TETE"), binning_file, lmax)
    cov[nbins:2*nbins, nbins:2*nbins] += so_cov.bin_mat(coupling_dict["TaPdPbTc"] * so_cov.chi(na, nd, nb, nc, ns, l, ps_all, nl_all, "TTEE"), binning_file, lmax)

    cov[2*nbins:3*nbins, 2*nbins:3*nbins] = so_cov.bin_mat(coupling_dict["PaPcPbPd"] * so_cov.chi(na, nc, nb, nd, ns, l, ps_all, nl_all, "EEEE"), binning_file, lmax)
    cov[2*nbins:3*nbins, 2*nbins:3*nbins] += so_cov.bin_mat(coupling_dict["PaPdPbPc"] * so_cov.chi(na, nd, nb, nc, ns, l, ps_all, nl_all, "EEEE"), binning_file, lmax)

    cov[nbins:2*nbins, :nbins] = so_cov.bin_mat(coupling_dict["TaTcTbPd"] * so_cov.chi(na, nc, nb, nd, ns, l, ps_all, nl_all, "TTTE"), binning_file, lmax)
    cov[nbins:2*nbins, :nbins] += so_cov.bin_mat(coupling_dict["TaPdTbTc"] * so_cov.chi(na, nd, nb, nc, ns, l, ps_all, nl_all, "TTET"), binning_file, lmax)

    cov[2*nbins:3*nbins, :nbins] = so_cov.bin_mat(coupling_dict["TaPcTbPd"] * so_cov.chi(na, nc, nb, nd, ns, l, ps_all, nl_all, "TTEE"), binning_file, lmax)
    cov[2*nbins:3*nbins, :nbins] += so_cov.bin_mat(coupling_dict["TaPdTbPc"] * so_cov.chi(na, nd, nb, nc, ns, l, ps_all, nl_all, "TTEE"), binning_file, lmax)

    cov[2*nbins:3*nbins, nbins:2*nbins] = so_cov.bin_mat(coupling_dict["PaTcPbPd"] * so_cov.chi(na, nc, nb, nd, ns, l, ps_all, nl_all, "EETE"), binning_file, lmax)
    cov[2*nbins:3*nbins, nbins:2*nbins] += so_cov.bin_mat(coupling_dict["TaPdTbPc"] * so_cov.chi(na, nd, nb, nc, ns, l, ps_all, nl_all, "EEET"), binning_file, lmax)
    ...
    cov = np.dot(np.dot(mbb_inv_ab, cov), mbb_inv_cd.T)

    np.save("%s/analytic_cov_%sx%s_%sx%s.npy" % (cov_dir, na, nb, nc, nd), cov)
