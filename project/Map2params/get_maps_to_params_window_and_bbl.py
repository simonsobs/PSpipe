...
for exp in experiment:
    ...
    if d['pixel_%s'%exp]=='CAR':
        ...
        vec=hp.pixelfunc.ang2vec(d['lon_%s'%exp],d['lat_%s'%exp], lonlat=True)
        disc=hp.query_disc(d['nside_%s'%exp], vec, radius=d['radius_%s'%exp]*np.pi/180)
        binary.data[disc]=1
    ...
    for f in freqs:
        ...
        window=so_window.create_apodization(binary, apo_type=d['apo_type_survey_%s'%exp], apo_radius_degree=apo_radius_degree)
