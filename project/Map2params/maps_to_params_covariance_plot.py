...
lcut={}
lmax_spec={}
lmax_spec['Planck_100']=1800
lmax_spec['Planck_143']=2000
lmax_spec['Planck_217']=2300
lmax_spec['LAT_93']=lmax
lmax_spec['LAT_145']=lmax
lmax_spec['LAT_225']=lmax
...
for sid1, spec1 in enumerate(spec_name):
    for sid2, spec2 in enumerate(spec_name):
        ...
        analytic_cov[n1,n2,n3,n4]=np.load('%s/analytic_cov_%sx%s_%sx%s.npy'%(cov_dir,n1,n2,n3,n4) )
        cov[n1,n2,n3,n4]=np.load('%s/mc_cov_%sx%s_%sx%s.npy'%(cov_dir,n1,n2,n3,n4))


        cut_lmax=np.amin([lmax_spec[n1],lmax_spec[n2],lmax_spec[n3],lmax_spec[n4]])
        
        new_nbin,new_lb,analytic_cov[n1,n2,n3,n4]=cut_matrix(analytic_cov[n1,n2,n3,n4],cut_lmax,lb)
        new_nbin,new_lb,cov[n1,n2,n3,n4]=cut_matrix(cov[n1,n2,n3,n4],cut_lmax,lb)
        ...
        for bl in ['TTTT','EEEE','TETE','TTEE','TEEE','TETT']:
            ...
            cov_select=so_cov.selectblock(cov[n1,n2,n3,n4], ['TT','TE','EE'],new_nbin,block=bl)
            analytic_cov_select=so_cov.selectblock(analytic_cov[n1,n2,n3,n4],  ['TT','TE','EE'],new_nbin,block=bl)
