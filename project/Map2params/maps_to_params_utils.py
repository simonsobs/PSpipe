...
def get_noise_matrix_spin0and2(noise_data_dir, exp, freqs, lmax, n_splits, lcut=0):
    
    """This function uses the noise power spectra computed by 'maps_to_params_prepare_sim_data'
    and generate a three dimensional array of noise power spectra [n_freq,n_freq,lmax] for temperature
    and polarisation.
    ...
    n_freq = len(freqs)
    nl_array_t = np.zeros((n_freq, n_freq, lmax))
    nl_array_pol = np.zeros((n_freq, n_freq, lmax))
...
def get_foreground_matrix(foreground_data_dir, fg_components, all_freqs, lmax):
    ...
    and generate a three dimensional array of foregroung power spectra [n_freq,n_freq,lmax].
    ...
    fl_array = np.zeros((n_freq, n_freq, lmax))
...
def generate_noise_alms(nl_array_t, lmax, n_splits, ncomp, nl_array_pol=None):
    ...
    note that nlms["T", i] is a (n_freq, size(alm)) array, it is the harmonic transform of
