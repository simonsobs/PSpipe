...
for c1,ar1 in enumerate(arrays):
    for c2,ar2 in enumerate(arrays):
        ...
        l,bl1= np.loadtxt(d['beam_%s'%ar1],unpack=True)
        ...
        for spec in ['TT','TE','ET','EE']:
            ...
            Dl_all['%s_%s'%(experiment,ar1),'%s_%s'%(experiment,ar2),spec]=bl1*bl2*ps_th[spec]
                    
            if ar1==ar2:
                DNl_all['%s_%s'%(experiment,ar1),'%s_%s'%(experiment,ar2),spec]=Nl[spec]*ns['%s'%(experiment)]
            else:
                DNl_all['%s_%s'%(experiment,ar1),'%s_%s'%(experiment,ar2),spec]=np.zeros(lmax)
                    

            Dl_all['%s_%s'%(experiment,ar2),'%s_%s'%(experiment,ar1),spec]=Dl_all['%s_%s'%(experiment,ar1),'%s_%s'%(experiment,ar2),spec]
            DNl_all['%s_%s'%(experiment,ar2),'%s_%s'%(experiment,ar1),spec]=DNl_all['%s_%s'%(experiment,ar1),'%s_%s'%(experiment,ar2),spec]
                
        spec_name+=['%s_%sx%s_%s'%(experiment,ar1,experiment,ar2)]
