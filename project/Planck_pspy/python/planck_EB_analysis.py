'''
This script analyze the planck EB spectra
'''
import matplotlib
matplotlib.use("Agg")
import numpy as np
...
for iii in range(iStart, iStop):
    ...
    for fpair in freq_pairs:
        ...
        if f0 != f1:
            ...
            ps_dict_sim["EB"] = (ps_dict_sim["EB"] +  ps_dict_sim2["EB"]) / 2
        ...
        vec_sim = np.append(vec_sim, ps_dict_sim["EB"][id])
...
for i, fpair in enumerate(freq_pairs):
    f0, f1 = fpair
    EB = vec_data[i * n_bins: (i+1) * n_bins]
    std_EB = std[i * n_bins: (i+1) * n_bins]
    ...
    plt.errorbar(lb,
                 ...
                 std_EB,
                 fmt=".",
                 label = r"%sx%s, $\chi^{2}$/dof = %0.2f/%d, PTE=%.02f" % (f0, f1, chi2, n_bins, PTE))
    plt.legend()
    np.savetxt("%s/EB_legacy_%sx%s.dat" % (mc_dir, f0, f1), np.transpose(np.array([lb, EB, std_EB])))
    
plt.savefig("%s/EB.png" % plot_dir, bbox_inches="tight")
plt.clf()
plt.close()
