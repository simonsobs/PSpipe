...
for sv in d["surveys_to_calib"]:
    for ar in d[f"arrays_{sv}"]:
        ...
        array = f"{sv}_{ar}"
        if array == d[f"ref_arrays_{array}"]: continue
        ref_array = d[f"ref_arrays_{array}"]
        ...
        lmin, lmax = d[f"ell_range_poleff_{array}"]
        id = np.where((lb >= lmin) & (lb <= lmax))[0]
        consistency.plot_residual(lb[id], res_spectrum[id], {"analytical": res_cov[np.ix_(id, id)]},
        ...
        results_dict[array] = {"multipole_range": d[f"ell_range_poleff_{array}"],
                            "ref_array": [f"ref_arrays_{array}"],
