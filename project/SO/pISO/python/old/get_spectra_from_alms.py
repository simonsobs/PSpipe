...
for sv in surveys:
    ...
    for ar in arrays[sv]:
        ...
        for k, map in enumerate(maps):
            master_alms[sv, ar, k] = np.load(f"{alms_dir}/alms_{sv}_{ar}_set{k}.npy")
            log.debug(f"{alms_dir}/alms_{sv}_{ar}_set{k}.npy")
...
if apply_kspace_filter:
    ...
    if kspace_tf_path == "analytical":
        if 'single_mask' in d.keys():
            single_mask = d['single_mask']
        else:
            single_mask = False
        kspace_transfer_matrix = kspace.build_analytic_kspace_filter_matrices(surveys,
                                                                              ...
                                                                              binning_file,
                                                                              lmax)
        for key in kspace_transfer_matrix.keys():
            # kspace_transfer_matrix[key] = np.diag(kspace_transfer_matrix[key].diagonal() + 0.0001)
            if 0. in kspace_transfer_matrix[key].diagonal():
                log.info(f'0. in kspace_transfer_matrix {key}')
                kspace_transfer_matrix[key] = np.diag(kspace_transfer_matrix[key].diagonal() + 0.0001)
            # np.save(f'test_kspace_transfer_matrix_{key}', kspace_transfer_matrix[key], allow_pickle=True)
    else:
...
for task in subtasks:
    ...
    # If this is true, skips all cross computations to prevent errors
    no_cross = (sv1 == sv2) & (nsplits_1 == 1)
    ...
    for spec in spectra:
        ...
        if ar1 == ar2 and sv1 == sv2 and not no_cross:
