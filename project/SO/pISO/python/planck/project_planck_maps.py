...
pspy_utils.create_directory(out_dir + '/npipe/')
pspy_utils.create_directory(out_dir + '/legacy/')

# Planck frequencies
planck_freqs = ["100", "143", "217", "353"]

template_name = d["planck_projection_template"]
template = so_map.read_map(template_name)
shape, wcs = template.data.geometry
print(shape, wcs)
# Define data directories
npipe_map_dir = d["npipe_map_dir"]
legacy_map_dir = d["legacy_map_dir"]
...
for task in subtasks:
    ...
    for split_npipe, split_legacy in zip(splits_npipe, splits_legacy):
        ...
        car_project.write_map(file_name=f"{out_dir}/npipe/{out_file_name}.fits")
        car_project.downgrade(8).plot(file_name=f"{out_dir}/npipe/{out_file_name}",
        ...
        ivar_project.write_map(file_name=f"{out_dir}/npipe/{out_file_name}.fits")
        ivar_project.downgrade(8).plot(file_name=f"{out_dir}/npipe/{out_file_name}",
                                    color_range=5e-4)
        ...
        car_project.write_map(file_name=f"{out_dir}/legacy/{out_file_name}.fits")

        car_project.downgrade(8).plot(file_name=f"{out_dir}/legacy/{out_file_name}",
                                    color_range=[300, 100, 100])
