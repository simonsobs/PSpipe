...
dory_path=/home/scw1361/sgiardie/tenki/point_sources


# Path to npipe maps & beams
map_path=/scratch/c.spxsg6/iso/dust/planck_projected
beam_path=/scratch/c.spxsg6/iso/dust/beams/npipe
...
mask_path=/scratch/c.spxsg6/dr6/masks/mask_galactic_equatorial_car_gal080_apo0_reverse.fits
...
for freq in ${freqs[@]}; do
  for split in ${splits[@]}; do
    ...
    time srun -n 10 -c 4 --cpu_bind=cores python "${dory_path}/dory.py" fit "${map_file}" "${ivar_file}" "${ps_catalog_path}" "${out_cat_path}" -R "tile:${tile_size}" -f "${freq}" -b "${beam_file}" --ncomp 3 --hack ${hack} -P ${prior} -m "${mask_path}" -s ${sigma_cat} --maxcorrlen 1000
    time srun -n 1 -c 40 --cpu_bind=cores python "${dory_path}/dory.py" subtract "${map_file}" "${out_cat_path}/cat.fits" "${out_map_file}" "${out_map_model_file}" -R "tile:${tile_size}" -f "${freq}" -b "${beam_file}" --ncomp 3 -s ${sigma_sub}
  done
done
