...
for freq1, freq2 in itertools.combinations_with_replacement(frequencies, r=2):
    if True in [freq1 in sv_ar for sv_ar in surveys_arrays_B]:
...
for f in spectra_auto:
    ...
    ax.legend()
    plt.savefig(save_path_cross_noises + f"noise_rms_{survey_B}x{survey_B}_{f}")
    plt.close()
...
for f in spectra_auto:
    for survey in d['surveys']:
        ...
        for ar in d[f'arrays_{survey}']:
            sv_ar2 = f'{survey}_{ar}'
        ...
        ax.legend()
        plt.savefig(save_path_cross_noises + f"noise_{survey}x{survey}_{f}")
        plt.close()
...
for f in spectra:
    if f != f[::-1]:
        ...
        ax.set_xlim(*plot_info["noise_BxB"]["xlims"][f])
        ax.legend()
        plt.savefig(save_path_cross_noises + f"corr_noise_{survey_B}x{survey_B}_{f}")
        plt.close()

# Define surveys and arrays to plot
for survey_A in ['dr6', 'planck']:
    if survey_A not in d['surveys']: continue
