...
sacc_fname = d["sacc_fname"] #read from dictionary
...
# Get Bbl: reading every combination
bbls = {}
for cross in spec_name_list:
    bbl = np.load(os.path.join(mcm_dir, f"{cross}_Bbl.npy"))
    if binned_mcm:
        # gets the spin0xspin0, spin0xspin2, spin2xspin0, and spin2xspin2 terms
        bbls[cross] = so_mcm.get_coupling_dict(bbl)
    else:
        bbls[cross] = bbl
...
#    cov = np.load(f"{cov_dir}/x_ar_{cov_name}_gp.npy")
    ...
    io.port2sacc(
        **common_kwargs, data_vec=data_vec, cov=cov, bbls=bbls, sacc_file_name=sacc_file_name, binned_mcm = binned_mcm
    )
