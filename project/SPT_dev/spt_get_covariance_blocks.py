...
for task in subtasks:
    ...
    if d["apply_alm_mask"] == True:
        ...
        lb, one_d_tf_ab = np.loadtxt(f"{spectra_dir}/one_dimension_alm_mask_tf_{na_r}x{nb_r}.dat", unpack=True)
        lb, one_d_tf_cd = np.loadtxt(f"{spectra_dir}/one_dimension_alm_mask_tf_{nc_r}x{nd_r}.dat", unpack=True)
        one_d_tf_ab = np.tile(one_d_tf_ab, 9)
        one_d_tf_cd = np.tile(one_d_tf_cd, 9)
