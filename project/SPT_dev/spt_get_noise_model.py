...
spectra_dir = "spectra_corrected_pspipe"
...
for sv in surveys:
    ...
    for id_ar1, ar1 in enumerate(arrays):
        for id_ar2, ar2 in enumerate(arrays):
            ...
            bb_ar1, bb_ar2  = {}, {}
            for field in ["T", "E", "B"]:
                lb_beam, bb_ar1[field] = pspy_utils.naive_binning(l, bl_ar1[field], binning_file, lmax)
                lb_beam, bb_ar2[field] = pspy_utils.naive_binning(l, bl_ar2[field], binning_file, lmax)

            id = np.where(lb_beam >= lb[0]) # use the same lmin

            print(lb)
            Rb = {}
            for spec in spectra:
                X,Y = spec
                nbs_ar1xar1[spec] *= bb_ar1[X][id] * bb_ar1[Y][id]
                nbs_ar1xar2[spec] *= bb_ar1[X][id] * bb_ar2[Y][id]
                nbs_ar2xar2[spec] *= bb_ar2[X][id] * bb_ar2[Y][id]
