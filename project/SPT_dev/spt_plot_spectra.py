...
for spec in ["TB", "EB", "BB"]:
    for spec_name in spec_name_list:
        ...
        lb, Db_redo = so_spectra.read_ps(f"{spec_dir}/Dl_{spec_name}_cross.dat", spectra=spectra)
    
        plt.plot(l_th[:lmax], ps_th[spec][:lmax], color="black")
        plt.plot(lb, Db_redo[spec], label=f"{spec} {spec_name} (uncorrected)")
        plt.legend()
...
for spec in ["TT", "TE", "EE"]:
    for spec_name in spec_name_list:
        ...
        lb, Db_redo = so_spectra.read_ps(f"{spec_corr_dir}/Dl_{spec_name}_cross_tf_bias_corr.dat", spectra=spectra)
        ...
        _, Db_th_redo, _, Db_th = get_binned_theory(l_th, ps_th, spec_name, spec, Bbl, lb, l_spt, plot_Bbl=False)
        

        l_spt, Db, sigmab, Db_th = l_spt[id_spt], Db[id_spt], sigmab[id_spt], Db_th[id_spt]
        lb_redo, Db_redo, Db_th_redo = lb[id_redo], Db_redo[spec][id_redo], Db_th_redo[id_redo]

        # take into account difference in binned theory
        if correct_bin_theory_diff == True:
            Db_redo -= (Db_th_redo - Db_th)
        ...
        ax[0].plot(lb_redo, Db_redo, label=f"SPT redo  {spec_name}, bias tf corrected")
        ax[0].legend()
        
        ax[1].errorbar(l_spt, l_spt*0)
        ax[1].errorbar(l_spt, Db - Db_redo, sigmab, label="SPT  - SPT redo ")
        ax[1].legend()
        ax[1].set_xlabel(r"$\ell$", fontsize=14)
        ax[1].set_ylabel(r"$D_\ell - D^{\rm redo}_\ell$", fontsize=14)

        ax[2].errorbar(l_spt, Db/Db_redo, lw=0.5, marker="o", ms=3, elinewidth=1, label=f"SPT/ SPT redo {spec_name}")
