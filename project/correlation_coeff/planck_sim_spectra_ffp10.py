...
for iii in subtasks:
    ...
    for freq_id,freq in enumerate(freqs):
        ...
        for hm,map,k in zip(splits,maps,np.arange(nSplits)):
            ...
            beamed_alms=sim_alm.copy()
            
            l,bl_T= np.loadtxt(d['beam_%s_%s_T'%(freq,hm)],unpack=True)
            l,bl_pol= np.loadtxt(d['beam_%s_%s_pol'%(freq,hm)],unpack=True)

            beamed_alms[0]=hp.sphtfunc.almxfl(beamed_alms[0],bl_T)
            beamed_alms[1]=hp.sphtfunc.almxfl(beamed_alms[1],bl_pol)
            beamed_alms[2]=hp.sphtfunc.almxfl(beamed_alms[2],bl_pol)
            
            for i in range(3):
                beamed_alms[i]=hp.sphtfunc.almxfl(beamed_alms[i],pixwin)

            pl_map=sph_tools.alm2map(beamed_alms,template)
            noise=so_map.read_map('%s/mc_noise/%s/ffp10_noise_%s_%s_map_mc_%05d.fits'%(ffpDir,freq,freq,hm,iii))
            pl_map.data+=noise.data
