...
arrays = ["pa5_f090", "pa6_f090"]
field = "TE"
lmax_beam = 6000
nsims  = 200

ps, mean, std, gamma, gamma_s = {}, {}, {}, {}, {}
for ar in arrays:
    ...
    bl_s = interp(l, l_s, bl_s)
    bl_TE_s = interp(l, l_s, bl_TE_s)
    bl_TB_s = interp(l, l_s, bl_TB_s)

    gamma_s[ar, "TE"] = bl_TE_s / bl_s
    gamma_s[ar, "TB"] = bl_TB_s / bl_s

    plt.plot(l, bl_s/bl_s[0])
    plt.plot(l, bl/bl[0])
    plt.show()

    plt.plot(l, gamma[ar, "TE"])
    plt.plot(l, gamma_s[ar, "TE"])
    plt.show()
...
plt.errorbar(lb, lb * null, std * lb, fmt=".", label=f"null {ar1} - {ar0}")
