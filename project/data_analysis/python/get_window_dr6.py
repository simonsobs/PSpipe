    ...
    if d[f"fill_holes_in_windows"] == True or d[f"fill_holes_in_maps"] == True:
        ...
        if d[f"fill_holes_in_maps"] == True:
            ...
            for k, map in enumerate(maps):
                ...
                coords_tholes = np.where((mask_without_holes.data[:] == 1.0) & (mask_with_holes.data[:]!=0.0) & (map_map.data[0,:,:]==0.0))
                coords_pholes = np.where((mask_without_holes.data[:] == 1.0) & (mask_with_holes.data[:]!=0.0) & (map_map.data[1,:,:]==0.0) & (map_map.data[2,:,:]==0.0))
