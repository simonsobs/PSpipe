...
#        my_masks["baseline"].data[:] *= extra_mask.data[:]
    ...
    if d[f"fill_holes_in_windows"] == True or d[f"fill_holes_in_maps"] == True:
        ...
        # make kspace versions
        kspace_mask_with_holes = mask_with_holes.copy()
        dist = so_window.get_distance(kspace_mask_with_holes, rmax=4 * rescale * np.pi / 180)
        kspace_mask_with_holes.data[dist.data < 0.5 * rescale] = 0
        kspace_mask_with_holes.data *= gal_mask.data
        
        kspace_mask_without_holes = mask_without_holes.copy()
        dist = so_window.get_distance(kspace_mask_without_holes, rmax=4 * rescale * np.pi / 180)
        kspace_mask_without_holes.data[dist.data < 0.5 * rescale] = 0
        kspace_mask_without_holes.data *= gal_mask.data
        
        # these are the holes we have effectively filled (if True above) in the (k-space) window:
        coords_holes = np.where((kspace_mask_without_holes.data[:] == 1.0) & (kspace_mask_with_holes.data[:]==0.0))
        ...
        if d[f"fill_holes_in_maps"] == True:
            ...
            for k, map in enumerate(maps):
                ...
                coords_tholes = np.where((kspace_mask_without_holes.data[:] == 1.0) & (kspace_mask_with_holes.data[:]!=0.0) & (map_map.data[0,:,:]==0.0))
                coords_pholes = np.where((kspace_mask_without_holes.data[:] == 1.0) & (kspace_mask_with_holes.data[:]!=0.0) & (map_map.data[1,:,:]==0.0) & (map_map.data[2,:,:]==0.0))
