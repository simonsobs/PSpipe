"""
generates simulation for customs nulls
"""
...
nlms_dir = "/pscratch/sd/x/xgarrido/sims/nlms/"
    ...
    for sv in surveys:
        ...
        for k in range(n_splits[sv]):
            for ar in arrays[sv]:
                ...
                if any("_out" in my_array for my_array in arrays[sv]):
                    noise_alm_file_name = f"{nlms_dir}/mnms_noise_alms_inout/mnms_nlms_inout_{ar}_set{k}_{iii:05d}.npy"
                else:
                    noise_alm_file_name = f"{nlms_dir}/mnms_noise_alms_{sv}/mnms_nlms_{sv}_{ar}_set{k}_{iii:05d}.npy"

                log.info(f"[Sim n° {iii}] read nlm noise file {noise_alm_file_name}")
                noise_alms = np.load(noise_alm_file_name)
