! compile with:
! FFLAGS="-fopenmp -fPIC -Ofast -ffree-line-length-none" f2py-2.7 -c -m mmcm_v7 mmcm_v7.f90  -lgomp
! computes mode mode coupling matrix with w3j and Wl input
!----------------------------------------------------------------------
! v1: nominal mode coupling matrix computation routines
! v2: w3j symbol computation incorporated into the routines
! v3: OMP implemented
! v4: ee/bb mode coupling matrix computed together
! v5: pure ee/bb added
! v6: deleted not needed lines and slicing (:) to (l1min:)
! v7: fixed EB and added function to compute all 6 mode coupling matrices
!----------------------------------------------------------------------
module mmcmatrix_v7
...
private
public mmcm_tt, mmcm_ee, mmcm_eebb, mmcm_pure_eebb, mmcm_te, mmcm_eb, DRC3JJ

integer, parameter :: sp = SELECTED_REAL_KIND(5,30)
integer, parameter :: dp = SELECTED_REAL_KIND(12,200)
!real(kind=4) :: pi = 3.141592653589793238462643383279502884197_sp
...
!***REVISION HISTORY  (07/29/14) got rid of dependencies to make it     
!                                self contained - SC                    
!   750101  DATE WRITTEN                                                
...
!***END PROLOGUE  DRC3JJ                                                
!                                                                       
      integer(kind=8) ::  L2, L3, M1, M2, M3, L1MIN, L1MAX , NDIM, IER 
      real(kind=8), intent(out) :: THRCOF(NDIM) 
!                                                                       
      INTEGER I, INDEX, LSTEP, N, NFIN, NFINP1, NFINP2, NFINP3, NLIM,   &
     &        NSTEP2                                                    
      DOUBLE PRECISION A1, A1S, A2, A2S, C1, C1OLD, C2, CNORM,          &
     &                 DENOM, DV, EPS, HUGE, L1, NEWFAC, OLDFAC,    &
...
subroutine mmcm_tt(lmax, w_tt, ell, m_tt)
   implicit none
   real(kind=8), dimension(0:lmax,0:lmax),intent(out) :: m_tt
   real(kind=8), dimension(0:lmax) :: w_tt
   integer(kind=8), dimension(0:lmax) :: ell
   integer(kind=8) :: lmax, l1, l2, l1min, l1max, m1, m2, ier, ndim
   real(kind=8), dimension(0:2*lmax+1) :: temp_w3j   
   real(kind=8), dimension(0:lmax) :: temp   
   m1 = 0
   m2 = 0
   ier = 0
   ...
   !$OMP PARALLEL
   !$OMP DO SCHEDULE(DYNAMIC) PRIVATE(l1min, l1max, ndim, temp_w3j, temp)
   do l2 = 0,lmax
      do l1 = 0,lmax
         ndim = max((l1+l2)+1, lmax+1)
         temp_w3j = 0.0
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_w3j, ndim, ier)

         temp(l1min:lmax) = temp_w3j(:lmax-l1min)

         m_tt(l1,l2)=(2*l2+1)/(4*pi)*sum((2*ell(l1min:)+1)*w_tt(l1min:)*temp(l1min:)*temp(l1min:))
   ...
   !$OMP END PARALLEL
   return
end subroutine mmcm_tt



subroutine mmcm_ee(lmax, w_ee, ell, m_ee)
   implicit none
   real(kind=8), dimension(0:lmax,0:lmax), intent(out) :: m_ee
   real(kind=8), dimension(0:lmax) :: w_ee
   integer(kind=8), dimension(0:lmax) :: ell
   integer(kind=8) :: lmax, l1, l2, l1min, l1max, m1, m2, ier, ndim
   real(kind=8), dimension(0:2*lmax+1) :: temp_w3j_p, temp_w3j_m
   real(kind=8), dimension(0:lmax) :: temp_p, temp_m
   ier = 0
   
   !$OMP PARALLEL PRIVATE(l1min, l1max, ndim, temp_w3j_p, temp_p, temp_w3j_m, temp_m, m1, m2)
   ...
   do l2 = 2,lmax
      do l1 = 2,lmax
         ...
         m1 = -2
         m2 = 2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_w3j_p, ndim, ier)
         temp_p(l1min:lmax) = temp_w3j_p(:lmax-l1min)
         
         temp_w3j_m = 0.0
         m1 = 2
         m2 = -2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_w3j_m, ndim, ier)
         temp_m(l1min:lmax) = temp_w3j_m(:lmax-l1min)      

         m_ee(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(l1min:lmax)+1)*w_ee(l1min:lmax)*&
         &(temp_p(l1min:lmax)+temp_m(l1min:lmax))*(temp_p(l1min:lmax)+temp_m(l1min:lmax)))
   ...
   m_ee(0:1,0:1) = 0.0
   return
end subroutine mmcm_ee

subroutine mmcm_eebb(lmax,w_ee,ell,m_ee,m_eebb)
   implicit none
   real(kind=8), dimension(0:lmax,0:lmax), intent(out) :: m_ee, m_eebb
   real(kind=8), dimension(0:lmax) :: w_ee
   integer(kind=8), dimension(0:lmax) :: ell
   integer(kind=8) :: lmax, l1, l2, l1min, l1max, m1, m2, ier, ndim
   real(kind=8), dimension(0:2*lmax+1) :: temp_w3j_p, temp_w3j_m
   ...
   do l2 = 2,lmax
      do l1 = 2,lmax
         ...
         m_ee(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(l1min:lmax)+1)*w_ee(l1min:lmax)*&
         &(temp_p(l1min:lmax)+temp_m(l1min:lmax))*(temp_p(l1min:lmax)+temp_m(l1min:lmax)))
         m_eebb(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(l1min:lmax)+1)*w_ee(l1min:lmax)*&
         &(temp_p(l1min:lmax)-temp_m(l1min:lmax))*(temp_p(l1min:lmax)-temp_m(l1min:lmax)))
   ...
   m_eebb(0:1,0:1) = 0.0
   return
end subroutine mmcm_eebb



subroutine mmcm_pure_eebb(lmax,w_ee,ell,m_ee,m_eebb)
   implicit none
   real(kind=8), dimension(0:lmax,0:lmax), intent(out) :: m_ee, m_eebb
   real(kind=8), dimension(0:lmax) :: w_ee
   real(kind=8), dimension(0:lmax) :: ell
   integer(kind=8) :: lmax, l1, l2, l1min, l1max, m1, m2, ier, ndim
   real(kind=8), dimension(0:2*lmax+1) :: temp_20p,temp_20m,temp_21p,temp_21m,temp_22p,temp_22m
   real(kind=8), dimension(0:lmax) :: j_20p,j_20m,j_21p,j_21m,j_22p,j_22m
   ier = 0

   !$OMP PARALLEL PRIVATE(l1min,l1max,ndim,j_20p,j_20m,j_21p,j_21m,j_22p,j_22m),&
   !$OMP & PRIVATE(temp_20p,temp_20m,temp_21p,temp_21m,temp_22p,temp_22m,m1,m2,l1,l2)
   !$OMP DO SCHEDULE(DYNAMIC)
   do l2 = 2,lmax
      do l1 = 2,lmax
         ndim = max((l1+l2)+1, lmax+1)
         ...
         m1 = -2
         m2 = 2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_20p, ndim, ier)
         j_20p(l1min:lmax+1) = temp_20p(:lmax+1-l1min)
         
         temp_20m = 0.0
         j_20m = 0.0
         m1 = 2
         m2 = -2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_20m, ndim, ier)
         j_20m(l1min:lmax+1) = temp_20m(:lmax+1-l1min)

         temp_21p = 0.0
         j_21p = 0.0
         m1 = -1
         m2 = 2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_21p, ndim, ier)
         j_21p(l1min:lmax+1) = temp_21p(:lmax+1-l1min)
         
         temp_21m = 0.0
         j_21m = 0.0
         m1 = 1
         m2 = -2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_21m, ndim, ier)
         j_21m(l1min:lmax+1) = temp_21m(:lmax+1-l1min)

         temp_22p = 0.0
         j_22p = 0.0
         m1 = 0
         m2 = 2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_22p, ndim, ier)
         j_22p(l1min:lmax+1) = temp_22p(:lmax+1-l1min)
         
         temp_22m = 0.0
         j_22m = 0.0
         m1 = 0
         m2 = -2
         call DRC3JJ(l1, l2, m1, m2, l1min, l1max, temp_22m, ndim, ier)
         j_22m(l1min:lmax+1) = temp_22m(:lmax+1-l1min)
                  

         
!         IF (l1 == 4 .AND. l2==4) THEN
!            print *,j_20p(4)+j_20m(4)
!            print *,j_20p(3)-j_20m(3)
!            
!            print *,j_21p(4)+j_21m(4)
!            print *,j_21p(3)-j_21m(3)
!            
!            print *,j_22p(4)+j_22m(4)
!            print *,j_22p(3)-j_22m(3)
!            
!         END IF
!         IF (l1 == 4 .AND. l2==4) THEN
!            print *,j_20p(4)
...
!         m_ee(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(:)+1)*w_ee(:)*                                           &
!         &((j_20p(:)+j_20m(:))+2*dsqrt((ell(:)+1)*ell(:)/((l1+2)*(l1-1)))*(j_21p(:)+j_21m(:))             &
!         &+dsqrt((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1)/((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)+j_22m(:)))* &
!         &((j_20p(:)+j_20m(:))+2*dsqrt((ell(:)+1)*ell(:)/((l1+2)*(l1-1)))*(j_21p(:)+j_21m(:))             &
!         &+dsqrt((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1)/((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)+j_22m(:))))
         
!         m_eebb(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(:)+1)*w_ee(:)*                                         &
!         &((j_20p(:)-j_20m(:))+2*dsqrt((ell(:)+1)*ell(:)/((l1+2)*(l1-1)))*(j_21p(:)-j_21m(:))             &
!         &+dsqrt((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1)/((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)-j_22m(:)))* &
!         &((j_20p(:)-j_20m(:))+2*dsqrt((ell(:)+1)*ell(:)/((l1+2)*(l1-1)))*(j_21p(:)-j_21m(:))             &
!         &+dsqrt((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1)/((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)-j_22m(:))))

         m_ee(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(:)+1)*w_ee(:)*((j_20p(:)+j_20m(:))+2*dsqrt(((ell(:)+1)*&
         &ell(:))/((l1+2)*(l1-1)))*(j_21p(:)+j_21m(:))+dsqrt(((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1))/&
         &((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)+j_22m(:)))**2)

         m_eebb(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(:)+1)*w_ee(:)*((j_20p(:)-j_20m(:))+2*dsqrt(((ell(:)+1)*&
         &ell(:))/((l1+2)*(l1-1)))*(j_21p(:)-j_21m(:))+dsqrt(((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1))/&
         &((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)-j_22m(:)))**2)
         
!         m_ee(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(:)+1)*w_ee(:)*                                           &
!         &((j_20p(:)+j_20m(:))+2*dsqrt((ell(:)+1)*ell(:)/((l1+2)*(l1-1)))*(j_21p(:)+j_21m(:))             &
!         &+dsqrt((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1)/((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)+j_22m(:)))**2)
!         
!         m_eebb(l1,l2)=(2*l2+1)/(16*pi)*sum((2*ell(:)+1)*w_ee(:)*                                         &
!         &((j_20p(:)-j_20m(:))+2*dsqrt((ell(:)+1)*ell(:)/((l1+2)*(l1-1)))*(j_21p(:)-j_21m(:))             &
!         &+dsqrt((ell(:)+2)*(ell(:)+1)*ell(:)*(ell(:)-1)/((l1+2)*(l1+1)*l1*(l1-1)))*(j_22p(:)-j_22m(:)))**2)
...
!   print *, m_ee/m_eebb
   return
end subroutine mmcm_pure_eebb



subroutine mmcm_te(lmax,w_te,ell,m_te)
   implicit none
   real(kind=8), dimension(0:lmax,0:lmax), intent(out) :: m_te
   real(kind=8), dimension(0:lmax) :: w_te
   integer(kind=8), dimension(0:lmax) :: ell
   integer(kind=8) :: lmax, l1, l2, l1min, l1max, m1, m2, ier, ndim
   real(kind=8), dimension(0:2*lmax+1) :: temp_w3j, temp_w3j_p, temp_w3j_m
   real(kind=8), dimension(0:lmax) :: temp, temp_p, temp_m
   ier = 0
   
   !$OMP PARALLEL PRIVATE(l1min, l1max, ndim, temp_w3j, temp, temp_w3j_p, temp_p, temp_w3j_m, temp_m, m1, m2)
   !$OMP DO SCHEDULE(DYNAMIC)
   do l2 = 2,lmax
      do l1 = 2,lmax
         ndim = max((l1+l2)+1, lmax+1)
         ...
         m_te(l1,l2)=(2*l2+1)/(8*pi)*sum((2*ell(l1min:lmax)+1)*w_te(l1min:lmax)&
         &*temp(l1min:lmax)*(temp_p(l1min:lmax)+temp_m(l1min:lmax)))
   ...
   m_te(0:1, 0:1) = 0.0
   return
end subroutine mmcm_te

subroutine mmcm_eb(lmax, w_eb, ell, m_eb)
   implicit none
   real(kind=8), dimension(0:lmax,0:lmax),intent(out) :: m_eb
   real(kind=8), dimension(0:lmax) :: w_eb
   integer(kind=8), dimension(0:lmax) :: ell
   integer(kind=8) :: lmax, l1, l2, l1min, l1max, m1, m2, ier, ndim
   real(kind=8), dimension(0:2*lmax+1) :: temp_w3j_p, temp_w3j_m
   real(kind=8), dimension(0:lmax) :: temp_p, temp_m
   ...
   do l2 = 2,lmax
      do l1 = 2,lmax
         ...
         m_eb(l1,l2)=(2*l2+1)/(4*pi)*sum((2*ell(l1min:lmax)+1)*w_eb(l1min:lmax)&
         &*temp_p(l1min:lmax)*temp_m(l1min:lmax))
   ...
   m_eb(0:1, 0:1) = 0.0
   return
end subroutine mmcm_eb



end module mmcmatrix_v7
